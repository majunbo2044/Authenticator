    ...
    switch (command) {
        ...
        case "autofill":
            ...
            if (okToInjectContentScript(tab)) {
                ...
                const siteName = getSiteNameFromTab(tab);
                ...
                if (!matchedEntries || matchedEntries.length === 0) {
                    chrome.tabs.sendMessage(tab.id, {
                        action: "text",
                        text: chrome.i18n.getMessage("autofill_no_match"),
                    });
                    break;
                }
                ...
                if (matchedEntries.length > 1) {
                    chrome.tabs.sendMessage(tab.id, {
                        action: "text",
                        text: chrome.i18n.getMessage("autofill_multiple_matches"),
                    });
                    break;
                }
                ...
                if (matchedEntries && matchedEntries.length === 1) {
                    ...
                    if (
                        ...
                        chrome.tabs.sendMessage(tab.id, {
                            action: "text",
                            text: chrome.i18n.getMessage("autofill_code_unavailable"),
                        });
                    }
                }
            }
