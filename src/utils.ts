export async function getSiteName() {
    const titleAndUrl = getSiteNameFromQuery();
    if (titleAndUrl) {
        return getSiteNameFromTitleAndUrl(titleAndUrl.title, titleAndUrl.url);
    }
    ...
    return getSiteNameFromTab(tab);
}

export function getSiteNameFromTab(tab: chrome.tabs.Tab) {
    const title = tab.title?.replace(/[^a-z0-9]/gi, "").toLowerCase() ?? null;
    return getSiteNameFromTitleAndUrl(title, tab.url ?? null);
}

function getSiteNameFromQuery() {
...
function getSiteNameFromTitleAndUrl(title: string | null, url: string | null) {
    if (!url) {
        return [title, null];
    }
